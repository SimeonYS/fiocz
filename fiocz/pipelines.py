...
class FioczPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fiocz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fiocz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fiocz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fiocz (date, title, link, content)"
